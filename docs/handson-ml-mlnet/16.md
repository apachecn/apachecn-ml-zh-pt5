...
# 十六、对 ML.NET 使用TensorFlow

在这一章中，我们将使用一个预先训练好的 TensorFlow 模型，特别是 Inception 模型，我们将把这个模型集成到一个**Windows Presentation Foundation**(**WPF**)应用中。我们将采用预先训练的模型，并通过添加一些食物和水体的图片来应用迁移学习。在迁移学习完成后，我们允许用户选择他们自己的图像。到本章结束时，您应该已经牢牢掌握了如何将TensorFlow模型集成到您的 model 应用中。
...
`ImageClassificationPredictor`类包含加载和预测初始TensorFlow模型所需的所有代码:
