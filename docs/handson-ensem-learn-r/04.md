...
我们认为一个数据集可以从 R 核心包`kernlab`中获得。该数据集是垃圾邮件，它有 4601 封电子邮件的集合，这些电子邮件带有标签，表明该电子邮件是垃圾邮件还是非垃圾邮件。该数据集收集了从电子邮件内容中获得的 57 个变量。任务是为垃圾邮件识别问题建立一个好的分类器。与前面的问题一样，数据集被快速划分为训练分区和验证分区:
