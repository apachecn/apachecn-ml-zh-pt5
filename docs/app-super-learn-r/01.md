<title>C12624_01_ePub_Final_SW</title>
...
**客户关系管理** ( **CRM** )应用程序中的交易数据大部分是表格形式，社交媒体中的信息主要是文本、音频、视频和图像。

我们可以将数据格式分类为结构化的——表格数据，如 CSV 和数据库表；非结构化—文本数据，如推文、FB 帖子和 word 文档；和半结构化。与机器很难处理和理解的文本不同，半结构化提供了关联的元数据，这使得计算机很容易处理它。它广泛用于许多 web 应用程序的数据交换，JSON 就是半结构化数据格式的一个例子。
...
虽然 vector 和 matrix 都是在程序中的各种计算中使用的有用结构，但它可能不足以存储真实世界的数据集，真实世界的数据集通常包含混合类型的数据，例如 CRM 应用程序中的 customer 表将客户姓名和年龄放在两列中。该列表提供了一种允许将两种不同类型的数据存储在一起的结构。
...
DataFrame 中的每一行都是紧密耦合的列集合的结果。每一列都清楚地定义了每行数据与其他每一行数据之间的关系。如果一列中没有相应的可用值，则用 n a 填充。例如，CRM 应用程序中的客户可能没有填写他们的婚姻状况，而其他一些客户填写了。因此，在应用程序设计过程中，指定哪些列是必需的，哪些是可选的就变得非常重要。
