...
**AdaBoost** 是最早也是最受欢迎的提升模型之一。在 AdaBoost 中，每棵新树都会根据先前树的误差调整其权重。通过调整以更高百分比影响这些样本的权重，更多地关注出错的预测。通过从错误中学习，AdaBoost 可以将弱学习器转变为强学习器。弱学习器是一种机器学习算法，其表现几乎不比机会好。相比之下，更强的学习器从数据中学到了相当多的东西，并且表现得相当好。
...
5.  重复步骤 2-4:随着过程的继续，残差应该从正负方向逐渐接近`0`。针对估计器的数量`n_estimators`继续迭代。
    ...
    这个过程可能会持续几十、几百或几千棵树。在正常情况下，你肯定会坚持下去。要把一个弱学习器转变成强学习器，需要的不仅仅是几棵树。因为我们的目标是理解梯度增强是如何在幕后工作的，然而，我们现在将继续讨论这个基本概念。
...
梯度推进回归器中的初始决策树被称为**基学习器**，因为它位于集成的基础。它是这个过程中的第一个学习器。术语*学习器*在这里表示*弱学习器*转变为*强学习器*。
