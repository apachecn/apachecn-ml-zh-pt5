...
在**文件**视图中，添加一个名为**离职-政策-培训**的文件夹，并在其中添加一个 **training.py** 文件，如图*图 9.7* 所示:
...
配置一个`AutoMLConfig`对象。您需要定义您想要投资的**任务类型**、**指标**、**培训数据**和**计算预算**。该过程的输出是一个模型列表，您可以从中选择最佳运行以及与该运行相关的最佳模型，如图*图 9.11* 所示:
...
训练数据可以在`training_data`参数中提供，或者以熊猫`Dataset`对象的格式。培训数据以表格形式显示，包括`target`栏。您定义想要预测的列的名称，传递`label_column_name`参数。默认情况下，AutoML 将使用该数据集对生成的模型进行定型和验证。如果数据集超过 20，000 行，数据集将被拆分，保留 10%用于验证。如果数据集小于 20，000 行，则使用交叉验证。如果您想指定从`training_data`创建多少个折叠，您可以使用`n_cross_validations`参数。另一种方法是提供`validation_size`参数，它是训练数据的百分比(值 *0.0* 到 *1.0* ),用作验证。如果您想要手动将数据分割成训练和验证数据，那么您可以将您的验证数据分配给`validation_data`参数，就像您将在本节后面所做的那样。
...
到目前为止，您已经创建了`experiment_config`，它包含了您将要执行的 **AutoML** 实验的配置。添加一个新单元，并将以下代码添加到中，启动 AutoML 培训流程:
