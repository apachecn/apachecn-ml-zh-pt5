...
需要注意的是，Optimus 将`None`和`NaN` ( **不是数字**)值视为可互换值，以表示空值。要处理它们，你可以做两件事:删除数据或估算数据。在这一节中，我们将展示 Optimus 如何帮助完成这两项任务，而不提供何时使用每种方法的详尽统计解释。让我们看看擎天柱如何帮助我们完成这两项任务。
...
擎天柱依靠的`impute`方法。使用这种方法，您可以轻松处理连续值。您可以应用以下四种技术之一来处理空值:
...
在擎天柱中，你可以轻松应用`modified_z_score`。让我们看看它是如何工作的。
...
谈论垃圾箱的工作原理很重要。擎天柱将包括最右边的边，但不包括左边的边。我们使用的面元[0，4，6，35]将表示为(0，4)，(4，6)，(6，35)。这意味着当宁滨时，Optimus 将不会为第一个 bin 取 0 值，但会为第二个 bin 取 4 值，也不会为第二个 bin 取 4 值，但会为第二个 bin 取 6 值:
