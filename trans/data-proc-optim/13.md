...
Optimus 的主要目标是帮助您找到处理数据的最简单方法，而不管 it 数据格式/来源或您可用的基础设施。考虑到这一点，我们将继续建立一个强大的方法和几乎所有的函数的基础，你将永远需要争论你的数据。

Bumblebee 是第二个明显的步骤，因为它提供了一个易于使用的 UI 来加载、探索和争论基于 Optimus 构建的数据。我们与大黄蜂的计划是继续保持我们对整个擎天柱项目的愿景。我们将继续向用户提供开源且易于使用的工具，以便他们能够尽可能快速、轻松地了解自己的数据。
