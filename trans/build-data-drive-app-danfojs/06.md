<title>B17076_04_ePub_RK</title>

# *第四章* : **数据分析、争论、转化**

**数据分析**、**争论**和**转换**是任何数据驱动项目的重要方面，作为一名数据分析师/科学家，你的大部分时间都将花在某种形式的数据处理上。虽然 JavaScript 是一种灵活的语言，具有操作数据结构的良好特性，但编写实用函数来一直执行数据争论操作是非常乏味的。因此，我们在 Danfo.js 中构建了强大的数据争论和转换功能，这可以大大减少在这个阶段花费的时间。
...
祝贺您完成本节的学习！数据转换和聚集是数据分析的非常重要的方面。有了这些知识，您就可以适当地转换和争论您的数据集。
...
在本章中，我们成功地介绍了 Danfo.js 中可用的数据转换和争论函数。首先，我们向您介绍了用于替换值、填充缺失值和检测缺失值的各种争论函数，以及将自定义函数应用于数据集的应用和映射方法。这些功能和技术的知识可确保您具备构建数据驱动型产品所需的基础，并从您的数据中获得洞察力。
