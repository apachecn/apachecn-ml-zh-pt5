...
在介绍了基础知识和**卷积神经网络**(**CNN**)背后的直觉/动机之后，我们将在可用于对象检测的最流行的数据集之一上演示这一点。我们还将看到 CNN 的初始层如何获得关于我们对象的非常基本的特征，但最终的卷积层将获得更多的语义级特征，这些特征是从第一层中的那些基本特征建立起来的。
...
在这一章中，我们将研究对象检测——找出图像中的对象。举个例子，想象一下一辆自动驾驶汽车需要像*图 11.1* 中那样检测路上的其他汽车。目标检测有很多复杂的算法。它们通常需要庞大的数据集、非常深的卷积网络和很长的训练时间。
