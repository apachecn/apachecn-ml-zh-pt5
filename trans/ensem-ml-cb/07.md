...
XGBoost 代表极端梯度增强。它是梯度提升机的一个变种，旨在提高性能和速度。Python 中的 XGBoost 库实现了梯度提升决策树算法。梯度提升这个名字来自于它的梯度下降算法，以尽量减少损失时，增加新的模型。XGBoost 可以处理回归和分类任务。
...
你需要在你的系统上安装`graphviz`库来绘制被增强的树。
